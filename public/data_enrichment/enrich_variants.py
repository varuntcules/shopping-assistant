    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                print(f"      ✗ JSON parsing error after {MAX_RETRIES} attempts")
                return None
...
def process_variant(variant: Dict, product_id: int, product_title: str, body_html: str, tags: str) -> Optional[Dict]:
    ...
    print(f"      Processing variant {variant_id}: {variant_title} (Price: {variant_price})")
...
def process_products():
    ...
    print("="*70)
    print("Product Variant Enrichment Service")
    print(f"Configuration: {PRODUCTS_TO_PROCESS} products, batch size: {BATCH_SIZE}")
    ...
    try:
        ...
        for idx, product in enumerate(products, 1):
            ...
            for variant in variants:
                ...
                if enriched_data:
                    try:
                        ...
                        # Show summary
                        confidence_score = enriched_data.get('confidence_score')
                        confidence_str = f" | Confidence: {confidence_score}/100" if confidence_score is not None else ""
                        print(f"      ✓ Enriched: {enriched_data.get('skill_level')} | {enriched_data.get('price_tier')} | {len(enriched_data.get('use_cases', []))} use cases{confidence_str}")
            ...
            if product_skipped > 0:
                print(f"  ✓ Completed: {product_processed} processed, {product_skipped} skipped")
            else:
                print(f"  ✓ Completed: {product_processed} variants processed")
